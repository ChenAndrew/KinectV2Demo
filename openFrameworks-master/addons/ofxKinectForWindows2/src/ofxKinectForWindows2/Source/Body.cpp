...
namespace ofxKinectForWindows2 {
	namespace Source {
		...
		HandPositions Body::CustomDrawFunction(int winX, int winY){
			ofPushStyle();
			HandPositions myHandPositions = {0,0,0,0};
			for(auto & body : bodies)
			{
				if (!body.tracked) continue;
				map<JointType,ofVec2f> HandMap;
				for(auto & part : body.joints)
				{
					//if(part.second.getType == Hand
					HandMap[part.second.getType()] = ofVec2f();
					HandMap[part.second.getType()].set(part.second.getProjected(coordinateMapper,ColorCamera));
				}

				HandMap[JointType_HandLeft][0] *= (float) winX/1920;
				HandMap[JointType_HandLeft][1] *= (float) winY/1080;

			    HandMap[JointType_HandRight][0] *= (float) winX/1920;
				HandMap[JointType_HandRight][1] *= (float) winY/1080;

				drawProjectedHand(body.leftHandState, HandMap[JointType_HandLeft]);
				drawProjectedHand(body.rightHandState, HandMap[JointType_HandRight]);
				myHandPositions.LeftHandX = HandMap[JointType_HandLeft][0];
				myHandPositions.LeftHandY = HandMap[JointType_HandLeft][1];
				myHandPositions.RightHandX = HandMap[JointType_HandRight][0];
				myHandPositions.RightHandY = HandMap[JointType_HandRight][1];
			}
			ofPopStyle();
			return myHandPositions;
		}
	}
}
